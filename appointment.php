...
$sqlfind = "DELETE FROM requestrepair WHERE user_name='$name_session'";
$result = mysqli_query($conn,$sqlfind) or die( mysqli_error($conn));

$sqlcheck = "SELECT repairID FROM appointment";
$res = mysqli_query($conn,$sqlcheck) or die( mysqli_error($conn));
if ($res->num_rows > 0) {
...
$id = "TR".$repairid;
$sqlinsert = "UPDATE appointment SET repairID='$id' WHERE user_name='$name_session' ";
$rezult = mysqli_query($conn,$sqlinsert) or die( mysqli_error($conn));
}
...
if ($result4->num_rows > 0) {
  ...
  while($id < (sizeof($row5_ar)-1)) {
      ...
      $costvals[] = $row8['cost_value'];
      $id = $id +1;
      $total = $total + $row8['cost_value'];
  }  
    $u = "";
    foreach($units as $uniting){
        $u .= $uniting. ",";
    }
    $cs = "";
    foreach($costshows as $csing){
        $cs .= $csing. ",";
    }
    $cv = "";
    foreach($costvals as $cving){
        $cv .= $cving. ",";
    }
    $sqlinsert = "INSERT into clientinvoice (user_name, unit, cost_show, cost_val) values ('$name_session','$u','$cs','$total')";
...
echo "Error: ".$sqlinsert."<br>". $conn->error;
     ...
     $sqlupdate = "UPDATE clientinvoice set technician_name='$techname' WHERE user_name='$name_session'";
...
; ?>
    <td style='color=black'><h3>Price</h3></td><td></td><td><input type="hidden" id="totalia"><h3 id="totaling"></h3></td>
<script>
